...
public class EventDispatcher {
    ...
    public EventDispatcher(List<EventHandler> handlers) {
        ...
        for (EventHandler handler : handlers) {
            ...
            if (this.handlers.containsKey(eventKey)) {
                throw new IllegalArgumentException("Duplicate handler registration for event '%s'".formatted(eventKey));
            }
    ...
    public void dispatch(EventMessage message) {
        ...
        try {
            ...
            log.error("Handler for event '{}' failed on payload {}", message.eventKey(), message.rawPayload(), e);
        }
    }
}
